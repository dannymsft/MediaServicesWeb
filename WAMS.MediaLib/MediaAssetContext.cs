...
namespace WAMS.MediaLib
{
    public class MediaAssetContext
    {
        ...
        {
            Table = WAMSConstants.GetTable(MediaAssetTableName);
        }


        public MediaAssetContext(CloudTableClient tableClient)
        {
            Table = tableClient.GetTableReference(MediaAssetTableName);
        }
        ...
        public bool DeleteAsset(MediaAsset asset)
        {
            try
            {
                ...
                if (tableResult.HttpStatusCode < 400) return true;
            }
            catch (Exception e)
            {
